class HomeController < ApplicationController
  ...
  def nearby_places
    ...
    @response = ActiveSupport::JSON.decode(@result.body)
    #if @response['featured_places'].present?
      #@offer_result = Typhoeus::Request.get("http://app-stg.hoppr.com/app/offer_detail.json", :params => {"place_id"=>"#{@featured_tile['place_id']}","offer_id"=>"#{@featured_tile['offer_id']}","session_id"=>"#{$redis.get('session_id')}","key"=>"android"})
      #@offer_details = JSON.parse(@offer_result.body)
    #end
  end
